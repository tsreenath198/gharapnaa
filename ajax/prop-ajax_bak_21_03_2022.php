	...
	switch($action):
		...
		case 'contactform' :
			...
			if($email!='' && $phone!='')
			{
				...
				$records = getCount($conn, $selectQuery);
		...
		case "sulogin":
			...
			$loginarray = (array)getRecordsAssoc($conn, "SELECT * FROM `".TBL_EMPLOYEES."` WHERE `user_name`='".$user."' AND `password`='".$encpwd."' AND `is_active`=1");
