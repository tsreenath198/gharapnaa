<?php
	...
	switch($action):
		case 'addcityform' :
			...
			$cityId = $db->escString($_REQUEST['loc_id']);
			$loc_name = $db->escString($_REQUEST['loc_name']);
			$state_id = $db->escString($_REQUEST['state_id']);
			$state_name = '';
			$district_id = $db->escString($_REQUEST['district_id']);
			$district_name = '';
			$country_id = $db->escString($_REQUEST['country_id']);
			$country_name = $db->escString($_REQUEST['country_name']);
			$is_active = (isset($_REQUEST['is_active']) && $_REQUEST['is_active']>0)? $_REQUEST['is_active'] : 0;
			$currDate = date("Y-m-d H:i:s");
			
			$returnUrl = ROOT.CITY_LIST.'1/';
			
			$cityDetails = $db->getSingleRowArray("SELECT `loc_id`, `loc_name` FROM `".TBL_LOCATIONS."` WHERE `loc_name`='".$loc_name."' AND `is_active`=1");
			if(count($cityDetails)==0 || (isset($cityDetails['loc_id']) && $cityDetails['loc_id']==$cityId))
			{
				if($state_id>0)
				{
					$stateDetails = $db->getSingleRowArray("SELECT `id`, `state_name` FROM `".TBL_STATES."` WHERE `id`='".$state_id."'");
					$state_name = $stateDetails['state_name'];
				}
				if($district_id>0)
				{
					$ditrictDetails = $db->getSingleRowArray("SELECT `id`, `district_name` FROM `".TBL_DISTRICTS."` WHERE `id`='".$district_id."'");
					$district_name = $ditrictDetails['district_name'];
				}
				if($cityId==0)
				{
					$insertCitySql = "INSERT INTO `".TBL_LOCATIONS."` (`loc_id`, `loc_name`, `loc_status`, `district_id`, `district_name`, `state_id`, `state_name`, `country_id`, `country_name`, `created_by`, `created_date`, `is_active`) VALUES (NULL, '".$loc_name."', '0', '".$district_id."', '".$district_name."', '".$state_id."', '".$state_name."', '".$country_id."', '".$country_name."', '".$userId."', '".$currDate."', '".$is_active."');";
					
					
					$cityId = $db->insertUpdateRecord($insertCitySql);
					if($cityId>0)
					{
						$status = array('status' => 'success', 'type' => 'text-success', 'message' => 'City added successfully.', 'redirectUrl' => $returnUrl);
					}
					else
					{
				...
				{
					//$cityDetails = $db->getSingleRowArray("SELECT `id`, `user_name`, `emp_code` FROM `".TBL_LOCATIONS."` WHERE `id`='".$empId."'");
					
					$updateEmpSQL = "UPDATE `".TBL_EMPLOYEES."` SET `loc_name`='".$loc_name."',`state_id`='".$state_id."',`state_name`='".$state_name."',`district_id`='".$district_id."',`district_name`='".$district_name."',`country_id`='".$country_id."',`country_name`='".$country_name."',`is_active`='".$is_active."' WHERE `loc_id`='".$cityId."'";
					$db->insertUpdateRecord($updateEmpSQL);
					
					$status = array('status' => 'success', 'type' => 'text-success', 'message' => 'City updated successfully.', 'redirectUrl' => $returnUrl);
				}
			}
			else
			{
				$status = array('status' => 'error', 'type' => 'text-danger', 'message' => 'City already existed.', 'redirectUrl' => $returnUrl);  /* 'Your email / phone already existed.' getErrorMsg($conn) */
			}
		...
		case 'getDistricts' :
			...
			if($stateId>0 && $session->getIsLoggedIn())
			{
				$districtData = $db->getRecordsArray("SELECT * FROM `".TBL_DISTRICTS."` WHERE `state_id`='".$stateId."' AND `is_active`=1");
			}
		...
		case 'addempform' :
			...
			$returnUrl = ROOT.LIST_EMPLOYEE.'1/';
			
			$empDetails = $db->getSingleRowArray("SELECT `id`, `user_name`, `emp_code` FROM `".TBL_EMPLOYEES."` WHERE `user_name`='".$user_name."' AND `is_active`=1");
			if(count($empDetails)==0 || (isset($empDetails['id']) && $empDetails['id']==$empId))
			{
				if($empId==0)
				{
					$directory = $function->createDirectoryPath('emp');
					$insertEmpSql = "INSERT INTO `".TBL_EMPLOYEES."` (`first_name`, `last_name`, `display_name`, `email`, `phone`, `user_name`, `password`, `password1`, `address`, `city`, `state`, `zip`, `department`, `emp_role`, `privilege`, `tl_id`, `created_by`, `created_date`, `directory`, `is_active`) VALUES ('".$first_name."', '".$last_name."', '".$display_name."', '".$email."', '".$phone."', '".$user_name."', '".$enc_password."', '".$password."', '".$address."', '".$city."', '".$state."', '".$zip."', '".$department."', '".$role."', '".$privilege."', '".$tl_id."', '".$userId."', '".$currDate."', '".$directory."', '".$is_active."');";
				...
				{
					$empDetails = $db->getSingleRowArray("SELECT `id`, `user_name`, `emp_code` FROM `".TBL_EMPLOYEES."` WHERE `id`='".$empId."'");
					
					if(count($empDetails)>0 && isset($empDetails['emp_code']) && $empDetails['emp_code']!='')
					{
						$empCode = $empDetails['emp_code'];
					}
					else
					{
						$empCode = $function->createEmpId($empId);
					}
					
					$updateEmpSQL = "UPDATE `".TBL_EMPLOYEES."` SET `first_name`='".$first_name."',`last_name`='".$last_name."',`display_name`='".$display_name."',`email`='".$email."',`phone`='".$phone."',`user_name`='".$user_name."',`password`='".$enc_password."',`password1`='".$password."',`address`='".$address."',`city`='".$city."',`state`='".$state."',`zip`='".$zip."',`department`='".$department."',`emp_role`='".$role."',`privilege`='".$privilege."',`tl_id`='".$tl_id."',`emp_code`='".$empCode."',`is_active`='".$is_active."' WHERE `id`='".$empId."'";
					$db->insertUpdateRecord($updateEmpSQL);
			...
			{
				$status = array('status' => 'error', 'type' => 'text-danger', 'message' => 'Username already eisisted. Try with another name.', 'redirectUrl' => $returnUrl);  /* 'Your email / phone already existed.' getErrorMsg($conn) */
			}
			echo json_encode($status);
			break;
...
$headers .= 'Cc: myboss@example.com' . "\r\n"; */
				...
				/* , 'otp' => $optNum */
				$status = array('status' => 'success', 'type' => 'text-success', 'message' => 'OTP sent successfully.');
			}
			else
			{
				/* , 'otp' => $optNum */
				$status = array('status' => 'error', 'type' => 'text-danger', 'message' => 'This email / mobile not registered.');
			}
		...
		case "enquiry":
			...
			/* $prpobj=new Mysqlidb(HOST_NAME,HOST_USER,HOST_PWD,HOST_DB_NAME);
		...
		case "sulogin":
			...
			if(count($loginarray)>0)
			{
				$sessionData = array(
					'id' => $loginarray["id"], 
					'name' => $loginarray["first_name"], 
					'user_id' => $loginarray["id"], 
					'email' => $loginarray["user_name"], 
					'phone' => $loginarray["phone"], 
					'is_admin' => ($loginarray["is_admin"]==1)? true : false, 
					'utype' => $loginarray["department"], 
					'urole' => $loginarray["emp_role"], 
					'directory' => $loginarray["directory"], 
					'profile_pic' => $loginarray["profile_pic"], 
					'display_name' => $loginarray["display_name"], 
					'tl_id' => $loginarray["tl_id"], 
					'login_type' => 'employee'
				);
					$session->setUserSession($sessionData);
					$view = $session->getLoadView();
					$view = str_replace("super/", "", $view);
					echo json_encode(array("sts"=>"01", "view" => $view));
			}
			else
			{
			...
			/* $loginobj=new Mysqlidb(HOST_NAME,HOST_USER,HOST_PWD,HOST_DB_NAME);
		...
		case "loginuser":
			...
			if(count($loginarray) >0)
			{
				if($loginarray["u_pwd"]==$encpwd)
				{
					...
					$sessionData = array(
						'id' => $loginarray["u_id"], 
						'name' => $loginarray["u_name"], 
						'user_id' => $loginarray["u_user_id"], 
						'email' => $loginarray["u_email"], 
						'phone' => $loginarray["u_phone"], 
						'directory' => $loginarray["u_directory"], 
						'profile_pic' => $loginarray["u_profile_pic"], 
						'display_name' => $loginarray["u_display_name"], 
						'is_admin' => false, 
						'utype' => 0, 
						'urole' => 0, 
						'tl_id' => 0, 
 ...
 */			break;
		...
		case "propertystatus":
			...
			{
				$updateSQL = "UPDATE `".TBL_PROPERTY."` SET `pr_status` = '".(($proStatus==1)? 1 : 0)."' WHERE `pr_id`='".$prid."'";
	...
	//echo json_encode(array($width, $height));
		...
		case "resetpwd":
			...
			$enpwd=md5($pwd);
			$loginobj=new MysqliDb(HOST,HOST_USER,PWD,DB);
		...
		case "addwishlist":
			...
			$uid=$_SESSION["UID"];
			$wlid=$_POST["wlid"];
			$wshobj=new Mysqlidb(HOST_NAME,HOST_USER,HOST_PWD,HOST_DB_NAME);
			if($wlid>0)
			{
