...
header("location:".ROOT."super/");
	...
	$countResult = $db->getSingleRowArray("SELECT COUNT(*) `totalRecords` FROM `".TBL_PROPERTY."` `tp` WHERE  `tp`.`pr_status`=1 ".$queryStr." ".$para);
	...
	$proarr = $db->getRecordsArray("SELECT `tp`.*, `pg`.`pr_img`, `pg`.`pr_id` `prid`, `tu`.`u_name`, `tu`.`u_email`, `tu`.`u_phone` FROM `".TBL_PROPERTY."` `tp` LEFT JOIN `".TBL_GALLERY."` `pg` ON `pg`.`pr_id`=`tp`.`pr_id`  LEFT JOIN `".TBL_USER."` `tu` ON `tu`.`u_id`=`tp`.`u_id` WHERE  `tp`.`pr_status`=1 ".$para.$queryStr." GROUP BY `tp`.`pr_id` ORDER BY `tp`.`pr_id` DESC LIMIT ".$recordsFrom.", ".RECORDS_LIMIT);
	
	//echo "SELECT `tp`.*, `pg`.`pr_img`, `pg`.`pr_id` `prid`, `tu`.`u_name`, `tu`.`u_email`, `tu`.`u_phone` FROM `".TBL_PROPERTY."` `tp` LEFT JOIN `".TBL_GALLERY."` `pg` ON `pg`.`pr_id`=`tp`.`pr_id`  LEFT JOIN `".TBL_USER."` `tu` ON `tu`.`u_id`=`tp`.`u_id` WHERE  `tp`.`pr_status`=1 ".$para.$queryStr." GROUP BY `tp`.`pr_id` ORDER BY `tp`.`pr_id` DESC LIMIT ".$recordsFrom.", ".RECORDS_LIMIT;
