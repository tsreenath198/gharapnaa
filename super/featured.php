...
//head();
	...
	if($cat>0)
	{
		...
		{
			case 1:
				$slug = ADMIN_FEATURED_APART;
				break;
			case 2:
				$slug = ADMIN_FEATURED_HOME;
				break;
			case 3:
				$slug = ADMIN_FEATURED_PLOT;
				break;
			case 4:
				$slug = ADMIN_FEATURED_VILLA;
				break;
			case 5:
				$slug = ADMIN_FEATURED_AGRI_LAND;
				break;
			default:
				$slug = ADMIN_FEATURED_APART;
				break;
	...
	$wishSelect = $wishJoin = '';
	/* if(getUserId()>0){
		$wishJoin .= " LEFT JOIN `".TBL_WISHLIST."` `tw` ON `tw`.`pr_id`=`tp`.`pr_id` AND `tw`.`u_id`='".getUserId()."' ";
		$wishSelect = ", `tw`.`wl_id`";
	} */
	...
	$countResult = $db->getSingleRowArray("SELECT COUNT(*) `totalRecords` FROM `".TBL_PROPERTY."` `tp` WHERE  `tp`.`pr_status`=1 AND `tp`.`pr_is_publish`=1 ".$queryStr);
	...
	$selectQuery = "SELECT `tp`.* ".$wishSelect." FROM `".TBL_PROPERTY."` `tp` ".$wishJoin." WHERE 1=1 ".$queryStr." AND `tp`.`pr_status`=1 AND `tp`.`pr_is_publish`=1 ORDER BY `tp`.`pr_id` DESC LIMIT ".$recordsFrom.", ".RECORDS_LIMIT;
