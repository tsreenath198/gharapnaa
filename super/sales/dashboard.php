<?php 
	...
	$propertySQL = $contactSQL = "";
	if(!$isMaster)
	{
		$sEmployee = $session->getUserId();
		$propertySQL .= " AND `emp_id`= '".$sEmployee."'";
		$contactSQL .= " AND `emp_id`= '".$sEmployee."'";
	}
	...
	$contPropetyEnqQuery = "SELECT COUNT(*) `totalRecords` FROM `".TBL_ENQUIRIES."` WHERE `eq_status`=1 ".$propertySQL;
	$propertyEnqRecords = $db->getSingleRowArray($contPropetyEnqQuery);
	
	$contEnqQuery = "SELECT COUNT(*) `totalRecords` FROM `".TBL_CONTACTS."` WHERE `status`=1 ".$contactSQL;
